...
var gBooks = [{
        "reviews": [{ id: '12345', readerName: 'nadav', rate: 4, readAt: '1.8.2018', txt: 'nice book loved it!' }],
        "id": "OXeMG8wNskc",
...
function getById(id, arr = gBooks) {
    return Promise.resolve(arr.find((item) => item.id === id))
}

function removeReviewById(id, bookId) {
    const booksCopy = [...gBooks]
    const bookIdx = gBooks.findIndex(book => book.id === bookId)
    const bookCopy = booksCopy[bookIdx]
    const reviewIdx = bookCopy.reviews.findIndex(review => review.id = id)
    bookCopy.reviews.splice(reviewIdx, 1)
    gBooks = booksCopy
    storageService.saveToStorage(KEY, gBooks)
    return Promise.resolve(bookCopy)
}
...
function addReview(review, bookId) {
    ...
    const bookIdx = gBooks.findIndex(book => book.id === bookId)
    const bookCopy = booksCopy[bookIdx]
    review.id = utils.makeId()
    if (bookCopy.reviews) bookCopy.reviews.unshift(review)
    else bookCopy.reviews = [review]
    gBooks = booksCopy
    storageService.saveToStorage(KEY, gBooks)
    return Promise.resolve(bookCopy)
}
...
function addGoogleBook(book) {
    ...
    const formatedBook = {
        ...
        catogories: [],
        description: 'need to get some info',
        id: book.id,
        language: bookInfo.language,
        listPrice: { amount: utils.getRndInt(10, 200), currencyCode: "ILS", isOnSale: (Math.random() > 0.5) ? true : false },
        pageCount: utils.getRndInt(100, 1000),
    ...
    const copy = [...gBooks]
    copy.unshift(formatedBook)
    gBooks = copy

    storageService.saveToStorage(KEY, gBooks)
}

function getNegsbyId(id) {
    const idx = gBooks.findIndex((book) => book.id === id)
    const prev = gBooks[idx - 1]
    const next = gBooks[idx + 1]
    ...
    return Promise.resolve(nextPrev)
        // return Promise.resolve({ nextId: next ? next.id : null, prevId: prev ? prev.id : null })
}