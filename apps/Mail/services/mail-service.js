...
var time = new Date(Date.now())
    // var stringiTime = time.toDateString()
var trash = []
const KEY = 'EMAILS'
var gMails = [
    { id: utils.makeId(), subject: 'programing', body: 'Sale applies to select Fitbit products \n and accessories on Fitbit.‌com while supplies last. Offer ends December 23,\n 2020 at 11:59 pm PST. Purchase is limited to ten (10) Fitbit products. Exclusions apply. Cannot be combined with other discounts or applied after the order has been placed. Terms of offer are subject to change. Void where prohibitedOvernight shipping means one (1) business ', isRead: true, isFav: true, sentAt: time },
    { id: utils.makeId(), subject: 'development', body: 'Humans aren’t perfectly objective judges. That’s not to say that C++ is objectively better than C\n, nor is it to say that C++ can do everything C can. The latter of which is untrue by the way. Modern C has a number\n of features C++ does not have, but the modern C fanboy will also be too often unable to point at what those features are, despite an overeager attitude to claim C is better.', isRead: true, isFav: true, sentAt: time },
    { id: utils.makeId(), subject: 'politics', body: 'It may be used positively in the context of a "political solution" which is \ncompromising and non-violent,[1] or descriptively as "the art or science of government", but also often carries a negative connotation.[2] For example, abolitionist Wendell Phillips declared that "we do not play politics; anti-slavery is no half-jest with us."[3] The concept has been defined in various ways, and ', isRead: false, isFav: true, sentAt: time },
    { id: utils.makeId(), subject: 'corona-virus', body: 'Coronaviruses are a group of related RNA viruses that cause diseases in mammals and birds. In humans and\n birds, they cause respiratory tract infections that can range from mild to lethal. Mild illnesses in humans include\n some cases of the common cold (which is also caused by other viruses, predominantly rhinoviruses), while more lethal varieties can cause SARS, MERS, and COVID-19.  .', isRead: true, isFav: true, sentAt: time },
    { id: utils.makeId(), subject: 'shit working', body: 'tore and/or access information on a device; Personalised ads display, personalised content, ad and content measurement, audience insights, and product development; Ensure security, prevent fraud, and debug; Technically deliver ads or content; Match and combine offline data sources; Link different devices; Receive and use automatically-sent device characteristics for identification .', isRead: true, isFav: false, sentAt: time },
    { id: utils.makeId(), subject: 'javascript', body: 'JavaScript (JS) is a lightweight, interpreted, or just-in-time compiled programming language with first-class functions. While it is most well-known\n as the scripting language for Web pages, MAScript standards are on yearly release cycles. This documentation refers to the latest We have put together a course that includes all the essential information you need to work towards your goal..', isRead: false, isFav: true, sentAt: time },
...
function query(name = '') {
    ...
    if (storageMails && !storageMails.length === 0) gMails = storageMails
...
function removeMail(id) {
    const currMailIdx = gMails.findIndex(mail => mail.id === id)
        // gMails[currMailIdx].isTrash=true
...
function submitCompose(newMail) {
    ...
    const mail = { id: utils.makeId(), ...newMail, isRead: false, sentAt: today }
    gMails.unshift(mail)
    storageService.saveToStorage(KEY, gMails)
}
...
function setFavs(id) {
    const currMail = getMailById(id)
    currMail.isFav = !currMail.isFav
        // storageService.saveToStorage(KEY, gMails)
    const favsMail = gMails.findIndex(mail => mail.id === id)
        // gMails[favsMail].isFav = !gMails[favsMail].isFav
    storageService.saveToStorage(KEY, gMails)
    return gMails[favsMail]
